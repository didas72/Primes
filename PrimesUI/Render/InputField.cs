...
namespace Primes.UI.Render
{
    internal class InputField : IRenderable, IUpdatable
    {
        ...
        {
            Text = string.Empty; FontSize = 20; Position = position; Size = size;  BackgroundColor = Color.GRAY; TextColor = Color.WHITE;
        }
        public InputField(string text, int fontSize, Vector2i position, Vector2i size, Color backgroundColor, Color textColor)
        {
            Text = text; FontSize = fontSize; Position = position; Size = size; BackgroundColor = backgroundColor; TextColor = textColor;
        }
        ...
        public void Render(Vector2i localOffset)
        {
            ...
            while (Raylib.MeasureText(lText, FontSize) > Size.x - 2) //trim to fit in given size
                lText = lText[1..];
            Raylib.DrawRectangle(Position.x + localOffset.x, Position.y + localOffset.y, Size.x, Size.y, BackgroundColor);
