    ...
    public class JobDistributer
    {
        ...
        public JobDistributer(ushort workerCount, string jobPath, string dumpPath)
        {
            ...
            for (int i = 0; i < workerCount; i++)
                Workers[i] = new Worker(dumpPath, jobPath, i) { primeBufferSize = Program.settings.GetUInt("PrimeBufferSize") };
        }
        ...
        {
            uint maxJobQueue = Program.settings.GetUInt("MaxJobQueue");
            Log.LogEvent($"Loading {maxJobQueue} jobs for processing.", "DistributingThread");

            try
            {
                return PrimesUtils.GetDoableJobs(jobPath, maxJobQueue, true);
            }
            catch (Exception e)
            {
                ...
                return PrimesUtils.GetDoableJobs(jobPath, maxJobQueue, false);
            }
        }
